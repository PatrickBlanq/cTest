...
const drawCapsule = () => {
    ...
    // 绘制矩形的右上角
    ctx.lineTo(x + props.data.width2 - borderRadius, y);
    ctx.arcTo(x + props.data.width2, y, x + props.data.width2, y + borderRadius, borderRadius);

    // 绘制矩形的右下角
    ctx.lineTo(x + props.data.width2, y + height - borderRadius);
    ctx.arcTo(x + props.data.width2, y + height, x + props.data.width2 - borderRadius, y + height, borderRadius);
    ...
    ctx.fillStyle = '#fff';
    ctx.fillText(props.data.middleNum, width / 2 - 8, height - 1);

    // 添加右边数字
    ctx.font = '0.8rem Microsoft YaHei';
    ctx.fillStyle = '#fff';
    ctx.fillText(props.data.rightNum, width + 18, height - 2);
};
</script>
